...
class _DynamicFormGeneratorState extends State<DynamicFormGenerator> {
  String _resolveCaption(FieldModel field) {
    final original = field.caption ?? '';
    print(original);

    if (radioValue == 1 || radioValue == 3) {
      if (original.trim() == 'نام /نام شرکت') {
        return 'نام';
      }
    }
    print("test");
    if (radioValue == 3) {
      if (original.trim() == 'کد ملی /شناسه شرکت') {
        return 'کد فراگیر';
      }

      if (original.trim() == 'محل تولد') {
        return 'کشور ';
      }
    }
  ...
  Widget _buildFormFields(FieldModel field, int index) {
    field = FieldModel.fromField(field)..caption = _resolveCaption(field);
    final caption = field.caption ?? '';

    if (radioValue == 3 &&
        field.type?.toLowerCase() == 'checkbox') {

      _values.remove(field.name); // جلوگیری از ارسال مقدار مخفی
    ...
    if (radioValue == 3 &&
      ...
      _values.remove(field.name); // جلوگیری از ارسال مقدار
      return const SizedBox.shrink();
    }
       ...
       return Container(
        ...
        child: Padding(
          ...
          child: RadioInputField(
            ...
            isRequired: isRequired, // ✅ الان درست کار می‌کند
          ),
        ),
      );
    }
          ...
          child: Column(
            children: [
              ...
              Padding(
                ...
                child:LoadingButton(text: 'ذخیره', onPressed: () async => await submitForm()
